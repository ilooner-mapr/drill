...
public class TestBitRpc extends ExecTest {
  ...
  public void testConnectionBackpressure(@Injectable WorkerBee bee, @Injectable final WorkEventBus workBus) throws Exception {
    ...
    final BootStrapContext c = new BootStrapContext(config1, SystemOptionManager.DEFAULT_VALIDATORS, ClassPathScanner.fromPrescan(config1));
    DrillConfig config2 = DrillConfig.create();
    BootStrapContext c2 = new BootStrapContext(config2, SystemOptionManager.DEFAULT_VALIDATORS, ClassPathScanner.fromPrescan(config2));
